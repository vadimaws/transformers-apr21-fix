...
def main():
    ...
    # THIS IS WHERE WE CHANGE THE CODE.
    column_names = ["INPUT_textbook_text", "OUTPUT_mc_question"]

    text_column = "INPUT_textbook_text" if "INPUT_textbook_text" in column_names else column_names[0]
    question_column = "OUTPUT_mc_question" if "OUTPUT_mc_question" in column_names else column_names[1]
    ...
    max_target_length = min(data_args.max_seq_length, tokenizer.model_max_length)
    ...
    def prepare_train_features(examples):
        ...
        for i in range(len(examples[text_column])):
            if examples[text_column][i] and examples[question_column][i]:
    ...
    # here: process the RACE dataset to have INPUT and OUTPUT columns the way we want them to.
    from RACE_dataset_HuggingFace import processing_RACE_df


    if training_args.do_train:
        if "train" not in raw_datasets:
            raise ValueError("--do_train requires a train dataset")
        train_dataset = raw_datasets["train"]  # .select(range(100))  # change.
        train_dataset = processing_RACE_df(train_dataset, prefix)
    ...
    if training_args.do_eval:
        ...
        eval_examples = raw_datasets["validation"]  # .select(range(20))  # change.
        eval_examples = processing_RACE_df(eval_examples, prefix)
    ...
    if training_args.do_predict:
        ...
        predict_examples = raw_datasets["test"]  # .select(range(20))  # change.
        predict_examples = processing_RACE_df(predict_examples, prefix)
    ...
    training_args.generation_max_length = (
        ...
        else data_args.max_answer_length
    )
...
"""
    ...
    kwargs = {"finetuned_from": model_args.model_name_or_path, "tasks": "question_generation"}
    if data_args.dataset_name is not None:
        kwargs["dataset_tags"] = data_args.dataset_name
        if data_args.dataset_config_name is not None:
            kwargs["dataset_args"] = data_args.dataset_config_name
